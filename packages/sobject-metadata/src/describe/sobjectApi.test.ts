...
describe("SObjectDescribeAPI", () => {
  ...
  beforeEach(async () => {
    ...
    sObjectAPI = new SObjectDescribeAPITestWrap(mockOrg);
  });
  ...
  it("should create the correct full URL for an sobject", () => {
    ...
    const expectedValue = `${
      mockOrg.getConnection().instanceUrl
  ...
  it("should create the correct sobject describe request (when last refresh date not specified)", () => {
    ...
    const expectedValue: RL.XHROptions = {
      ...
      headers: {
        ...
        Authorization: `OAuth ${mockOrg.getConnection().accessToken}`,
  ...
  it("should return sobject description array when reauth is needed and subsequent batch request is successful", async () => {
    const sObjectName = SObjectTestData.customSObject.name;
    const timestamp = "Fri, 07 Aug 2020 12:00:00 GMT";
    const xhrStub = sandboxStub.stub(RL, "xhr");
    const resp401: RL.XHRResponse = {
      status: 401,
      responseText: "ERROR",
      headers: {
        date: timestamp,
    ...
    xhrStub.onFirstCall().returns(Promise.reject(resp401));
    xhrStub.onSecondCall().returns(Promise.resolve(resp200));
    const expectedValue: DescribeSObjectResult[] = [
      { sObjectName, result: SObjectTestData.customSObject, timestamp },
    ];
    const actualValue = await sObjectAPI.describeSObjectBatch([sObjectName], 0);
  ...
  it("should return sobject description when reauth is needed and subsequent request is successful", async () => {
    const sObjectName = SObjectTestData.customSObject.name;
    const timestamp = "Fri, 07 Aug 2020 12:00:00 GMT";
    const xhrStub = sandboxStub.stub(RL, "xhr");
    const resp401: RL.XHRResponse = {
